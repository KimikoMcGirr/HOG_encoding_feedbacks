...
def open_ea_data(f, dir_to_use, model):
    all_params, last_params, all_mses, last_mses = get_sim_data(f, num_sims=2000)
    mses = []
    for param in last_params:
        mses.append(sum(model.calc_sim_score(param, exp_data, exp_time, ptpD=False)[:18]))
    re_idx = sorted(range(len(mses)), key=lambda k: mses[k])
    params = last_params[re_idx]
    mses = np.sort(mses)
    return mses, params



def load_ea_data(folders, dir_to_use, stripped_name, model_fxns, saved_data):
    ea_params = []
    ea_mses = []
    if saved_data:
        for f in folders:
            ...
            params = data["params"]
            mses = data["mses"]
            ea_params.append(np.array(params))
            ea_mses.append(np.array(mses))

    else:
        for f, model in zip(folders, model_fxns):
            mses, params = open_ea_data(f, dir_to_use, model)
            ea_params.append(params)
            ea_mses.append(mses)
            data_to_hdf5(dir_to_use+'_EA_data', stripped_name, params, mses)
    return ea_params, ea_mses


def use_EA_params(model_fxns, folders, dir_to_use, stripped_name, ei, particle_num, saved_data=False):
    #grab EA data
    ea_params, ea_mses = load_ea_data(folders, dir_to_use, stripped_name, model_fxns, saved_data)
    ...
    while len(mses[0]+mses[1]+mses[2]) < particle_num:
        model = select_model(model_fxns)
        idx = model_fxns.index(model)
        priors = ea_params[idx]
        # print(priors)
        params = model.select_params(priors[:500])
...
def main(particle_num, model_fxns, eis):
    ...
    params, mses, c_M2b, c_M2c, c_M3 = use_EA_params(model_fxns, folders, dir_to_use, stripped_name, eis[0], particle_num, saved_data=True)

    data_to_hdf5(dir_to_use, stripped_name, params, mses, c_M2b, c_M2c, c_M3)
    for i, ei in enumerate(eis[1:]):
        print("Now running schedule #" + str(i+1) + " Ei=" + str(ei))
        params, mses, c_M2b, c_M2c, c_M3 = run_priored_schedule(model_fxns, params, ei, particle_num)
        data_to_hdf5(dir_to_use, stripped_name, params, mses, c_M2b, c_M2c, c_M3)
...
if __name__ == '__main__':
    ...
    # ea_base = 'C:/Users/sksuzuki/Desktop/killdevil/runs_for_paper/'
    # folders = [ea_base+'191205_kb_M2b/', ea_base+'191205_kb_M2c/', ea_base+'190926_kb_M4_long/']
    ea_base = 'C:/Users/sksuzuki/Documents/GitHub/HOG_encoding_feedbacks/python_modules/ABC_SMC/200330_M2b_M2c_M3_model_selection_EA_data/'
    folders = [ea_base+'200330_M2b_M2c_M3_model_selection_0000.hdf5', ea_base+'200330_M2b_M2c_M3_model_selection_0001.hdf5', ea_base+'200330_M2b_M2c_M3_model_selection_0002.hdf5']
...
# define model functions
    ...
    save_filename = '200330_M2b_M2c_M3_model_selection.txt'
    # number_eas = 500
    particle_num = 100
