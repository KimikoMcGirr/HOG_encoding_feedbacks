...
save_filename = '190118_1D_a1_map2k_pulse.txt'

# wt_folder = '../data/MAPK activation/WT'
# t100a_folder = '../data/MAPK activation/T100A'

# for cvs file in folder
# load csv
# save data in scorefxn data
# save time in scorefxn time
# return scorefxn data and time

base_folder = 'C:/Users/sksuzuki/Desktop/killdevil/data_pbs2/MAPK activation/'
# pathlib.Path(_folder).glob('**/*.csv')
...
def get_data(base_folder):
    ...
    for i, data_folder in enumerate(pathlib.Path(base_folder).glob('**/'), start =1):
        ...
        if dose_data:
            experiment_data.append(dose_data)
            experiment_time.append(time)
    return (experiment_data, experiment_time)
...
# hog1_pulse_folder = 'C:/Users/sksuzuki/Desktop/killdevil/data_pbs2/MAPK activation/pulse_hog1/'
# pbs2_pulse_folder = 'C:/Users/sksuzuki/Desktop/killdevil/data_pbs2/MAPK activation/pulse_pbs2/'
...
# hog1_pulse_folder = '/nas/longleaf/home/sksuzuki/HOG_model/data_pbs2/MAPK activation/pulse_hog1'
# pbs2_pulse_folder = '/nas/longleaf/home/sksuzuki/HOG_model/data_pbs2/MAPK activation/pulse_pbs2'
...
# mapk_time, mapk_pulse_data = load_csv_data(hog1_pulse_folder)
# mapk_time, map2k_pulse_data = load_csv_data(pbs2_pulse_folder)
...
class experiment():
    def __init__(self, doses, exp_data,
                    args.number_of_generations, args.number_of_individuals):

        self.doses = doses
        self.exp_data = exp_data

        self.number_of_generations = 10
        self.number_of_individuals = 4
...
# signal strengths (uM)
doses = {"all":[0, 50000, 150000, 250000, 350000, 450000, 550000],
        "low high":[150000, 550000],
        "pulse":[550000]
    }
...
def b3_a1_1D_X(initals,t,total_protein,sig,params):
...
def simulate_wt_experiment(inits, total_protein, sig, learned_params, time):
    ...
    #solve odes:
    odes = odeint(b3_a1_1D_X, inits, time, args=(total_protein, sig, learned_params))
...
def scorefxn1(scorefxn_data, inits, params_constants,
    ...
    for data, time in zip(scorefxn_data, scorefxn_time):
...
def scorefxn_helper(individual):
    ...
    return scorefxn1(scorefxn_data, inits, params_constants, individual, scorefxn_time),
