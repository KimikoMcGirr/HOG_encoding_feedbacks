# import matplotlib
# matplotlib.use('Agg') # for plotting on the cluster
import logging
...
import warnings
import scipy.io # To import the raw data
import copy
import pandas as pd
import pathlib
import math
...
scorefxn_data = [mapk_wt_data, mapk_t100a_data, map2k_wt_data, map2k_t100a_data]
...
# # list of parameters to solve for
# param_list = ['k_stim', 'gamma', 'delta', 's1', 's2', 'X0', 'F0', 'k_n', 'k_s', 'eps', 'XYtot']
#
# bounds = [(0,10.), (0.,10.), (0.,10.), (0.,10.), (0.,10.), (0.,10.), (0.,10.), (0.,10.), (0.,10.), (0.,10.), (0.,10.)]
#
# # sample parameters (from Colmes paper)
# params = {'k_stim' : 0.25,
#           'gamma': 1.,
#           'delta': 1.,
#           's1': 0.5,
...
#     return [params[u] for u in param_list]
...
def scorefxn1(scorefxn_data, inits, params_constants,
    ...
    for sig, MAPK_wt_data, MAPK_t100a_data in zip(params_constants[-1],
        ...
        for fxn, exp_data, map2k_data in zip([simulate_wt_experiment,simulate_t100a_experiment],[MAPK_wt_data, MAPK_t100a_data], [map2k_wt_data, map2k_t100a_data]):
...
if __name__ == '__main__':
    ...
    # number of individuals, generations, mutation probability, and the scoring method
    parser.add_argument("--n_individuals", default = "100")
    parser.add_argument("--n_gen", default = "100")
    parser.add_argument("--mutpb", default = "0.1")
    parser.add_argument("--use_power", action = "store_true")

    # output stuff
    parser.add_argument("--ofile", default = "fits.hdf5")
    parser.add_argument("--odir", default = "most_fit")
    ...
    # make folder to visualize results in, if its there clear it out
    if comm.rank == 0:
        if not os.path.exists(args.odir):
            os.mkdir(args.odir)
        else:
            os.system('rm {0}'.format(os.path.join(args.odir, '*')))

    # set the mutation probability
    mutpb = float(args.mutpb)

    # open the write file (only rank == 0 does this)
    if comm.rank == 0:
    ...
    if comm.rank == 0:
        #EVOLUTION!
        population = toolbox.population(n=int(args.n_individuals))
    ...
    for gen in range(NGEN):
        ...
        # generate the offspring (should be equivalent to eaSimple)
        if comm.rank == 0:
            logging.debug('0: working on generation {0}'.format(gen))
            offspring = algorithms.varAnd(population, toolbox, cxpb = 0.5, mutpb = mutpb)
        else:
            offspring = None

        # distribute the offspring to every node
        comm.Barrier()
        ...
        # split the offspring amongst the proccessors
        for ix in range(comm.rank, len(offspring), comm.size):
            logging.debug('{0}: simulating system for individual {1}'.format(comm.rank, ix))

            score = toolbox.evaluate(offspring[ix])
            # store index, correlation coefficient, and the simulate Rho concentration over time
            result.append((ix, score))

            logging.debug('{0}: got score of {1}'.format(comm.rank, score))
        ...
        if comm.rank==0:
            ...
            for eachpart in data:
                final_result.extend(eachpart)

            final_result = sorted(final_result, key = lambda u: u[0])

            fits = [u[1] for u in final_result]
            # X_sims = np.array([u[2] for u in final_result])

            for fit, ind in zip(fits, offspring):
                ind.fitness.values = (fit,)
            ofile.create_dataset('{0}/params'.format(gen), data = np.array(offspring))
            ofile.create_dataset('{0}/score'.format(gen), data = np.array(fits))
            # ofile.create_dataset('{0}/X_sims'.format(gen), data = X_sims)

            # plot the most fit in this generation
            # fig, ax = plt.subplots(nrows = 1)

            # ax.imshow(final_result[np.argmin(fits)][2])

            #print 'writing to file, gen {0}'.format(gen)
            # plt.savefig(os.path.join(args.odir, 'generation_{0:03d}.png'.format(gen, comm.rank)), dpi = 100)
            # plt.close()

            logging.debug('0: got mean fitness of {0}'.format(np.mean(fits)))
