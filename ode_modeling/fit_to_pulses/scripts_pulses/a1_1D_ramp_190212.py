...
save_filename = '190218_a1_map2k_ramp.txt'
...
ramp_folder = '/nas/longleaf/home/sksuzuki/HOG_model/data_pbs2_branches/MAPK activation/ramp_1'
...
# ramp_folder = 'C:/Users/sksuzuki/Desktop/killdevil/data_pbs2_branches/MAPK activation/ramp_1'
...
experiment_data = {0 : [mapk_wt_data[0], mapk_t100a_data[0]],
                    ...
                    550000 : [mapk_wt_data[6], mapk_t100a_data[6], map2k_wt_data[1], map2k_t100a_data[1]],
                    'ramp' : ramp_data
    }
    ...
    for dose in experiment_doses:
        ...
        for i, fxn in enumerate(experiment_fxns):
            ...
            if dose == 150000 | dose == 550000:
                ...
                error_active = ((exp_data[i+1] - map2k)**2).mean()
                mse_total += error_active

    exp_data = experiment_data.get('ramp')
    exp_time = experiment_time.get('ramp')
    data = simulate_wt_experiment(inits, total_protein, dose, arr_params_IP, exp_time, signal_ramp_special, signal_params)
    mapk = data[:,2]/total_protein[2]*100
