...
def plt_param_behaviors(model_fxns, top_params, plt_top, params_constants, initials,  doses, time, param,
                        mapk_wt_data=None, mapk_t100a_data=None, mapk_time=None, ptp23D=False, ss=False, plt_bad=0,
                        save_fig=''):
    plt.clf()
    font = {'family' : 'Arial',
        'weight' : 'normal',
...
#######################
        ...
        #     # ptpD_total_protein = params_constants #lessen ptp affect
        #     # ptpD_inits = initials
        #
        ...
        #         for sig in doses:
        #             odes = simulate_ptpD_experiment_M3c(model_fxns.m, ptpD_ss_inits, ptpD_total_protein, sig, params, time)
        #             active = odes[:,param]/params_constants[param]*100
        #             ax1.plot(time, active, color=palette.get(sig))
        #             data = model_fxns.t100a(model_fxns.m, ptpD_ss_inits, ptpD_total_protein, sig, params, time)
...
def plt_mses_gen(gen, mse, idx_top, save_fig=''):
    plt.clf()
...
def plt_inhib_cis(sims, time, ci=95, #hog1_ramp_data=None, mapk_ramp_time=None,
                        save_fig=''):
...
import warnings
def nopos_to_list(model_fxns, top_params, params_constants, initials, time, param,
                        ss = False):
    ...
    for idx, params in enumerate(top_params):
        if ss:
            ...
            try:
                ...
                data = simulate_wt_experiment(model_fxns.nopos, ss_data, params_constants, 350000, params, time)
                active = data[:,2]/params_constants[2]*100
                sims.append(active)
            except RuntimeWarning:
                print("Runtimewarning at idx: "+str(idx))
...
# def plt_nopos_cis(sims, time, ci=95,
#                         save_fig=''):
#
#     fig, (ax1) = plt.subplots(1, 1, figsize=(5,4))
#
...
#     if save_fig:
#         plt.savefig(save_fig, dpi=300,bbox_inches='tight')
# def plt_param_cis_fig3c( model_fxns, top_params, params_constants, initials, time,
#                         exp_data=None, exp_time=None, ss=False, t100a=False, ptpD=False, ci= 95,
#                         save_fig=''):
#
#     doses= [150000,550000]
#     param=2
#
#     fig, ax = plt.subplots(2, 3, figsize=(6,4), sharex='all', sharey='all')
#     font = {'family' : 'Arial',
#         'weight' : 'normal',
#         'size'   : 16}
#     plt.rc('font', **font)
...
#
#     for sig in doses:
#         sims = fit_data_to_list(model_fxns, top_params[0], params_constants, initials, time, param, sig, t100a=True, ptpD=False, ss=ss)
#         ax[0, 0] = sns.tsplot(sims, time,  ci = ci, color=palette.get(sig), dashes=dashes)
#     for sig in doses:
#         sims = fit_data_to_list(model_fxns, top_params[1], params_constants, initials, time, param, sig, t100a=True, ptpD=False, ss=ss)
#         ax[0, 1] = sns.tsplot(sims, time,  ci = ci, color=palette.get(sig), dashes=dashes)
#     for sig in doses:
#         sims = fit_data_to_list(model_fxns, top_params[2], params_constants, initials, time, param, sig, t100a=True, ptpD=False, ss=ss)
#         ax[0, 2] = sns.tsplot(sims, time,  ci = ci, color=palette.get(sig), dashes=dashes)
#     for sig in doses:
#         sims = fit_data_to_list(model_fxns, top_params[0], params_constants, initials, time, param, sig, t100a=False, ptpD=False, ss=ss)
#         ax[1, 0] = sns.tsplot(sims, time,  ci = ci, color=palette.get(sig), dashes=dashes)
#     for sig in doses:
#         sims = fit_data_to_list(model_fxns, top_params[1], params_constants, initials, time, param, sig, t100a=False, ptpD=False, ss=ss)
#         ax[1, 1] = sns.tsplot(sims, time,  ci = ci, color=palette.get(sig), dashes=dashes)
#     for sig in doses:
#         sims = fit_data_to_list(model_fxns, top_params[2], params_constants, initials, time, param, sig, t100a=False, ptpD=False, ss=ss)
#         ax[1, 2] = sns.tsplot(sims, time,  ci = ci, color=palette.get(sig), dashes=dashes)
#
#     mstyles = {0: ['full'],
#                 1: ['full', 'bottom'],
#                 2: ['^', 's', '^', '<', '>', 's', 'X'],
...
#                 4: ['full']}
#
#     for sig, data, mark in zip(doses, exp_data[0], mstyles.get(param)):
#         if sig == 0:
#             mark = 'o'
#         ax[0,0].plot(exp_time, data, marker=mark, markersize=6, linestyle="-", fillstyle='none', color=palette.get(sig), mec='black', label = str(int(sig/1000))+'mM KCl')
#         ax[0,1].plot(exp_time, data, marker=mark, markersize=6, linestyle="-", fillstyle='none', color=palette.get(sig), mec='black', label = str(int(sig/1000))+'mM KCl')
#         ax[0,2].plot(exp_time, data, marker=mark, markersize=6, linestyle="-", fillstyle='none', color=palette.get(sig), mec='black', label = str(int(sig/1000))+'mM KCl')
#
#     for sig, data, mark in zip(doses, exp_data[1], mstyles.get(param)):
#         if sig == 0:
#             mark = 'o'
#         ax[1,0].plot(exp_time, data, marker=mark, markersize=6, linestyle="-", fillstyle='none', color=palette.get(sig), mec='black', label = str(int(sig/1000))+'mM KCl')
#         ax[1,1].plot(exp_time, data, marker=mark, markersize=6, linestyle="-", fillstyle='none', color=palette.get(sig), mec='black', label = str(int(sig/1000))+'mM KCl')
#         ax[1,2].plot(exp_time, data, marker=mark, markersize=6, linestyle="-", fillstyle='none', color=palette.get(sig), mec='black', label = str(int(sig/1000))+'mM KCl')
#
#
#     ax[0,0].grid(color='grey', linestyle='-', axis='y', linewidth=1)
#     ax[0,1].grid(color='grey', linestyle='-', axis='y', linewidth=1)
#     ax[0,2].grid(color='grey', linestyle='-', axis='y', linewidth=1)
#     ax[1,0].grid(color='grey', linestyle='-', axis='y', linewidth=1)
#     ax[1,1].grid(color='grey', linestyle='-', axis='y', linewidth=1)
#     ax[1,2].grid(color='grey', linestyle='-', axis='y', linewidth=1)
#
#     if param == 3:
...
#
#
# def plot_figure_3c(model_fxns, sim_data, params_constants, initials, time, exp_datas, exp_time):
#
#     fig, ax = plt.subplots(2, 3, figsize=(6,4), sharex='all', sharey='all')
#     plt_param_cis_fig3c(ax[0, 0], model_fxns, sim_data[0], params_constants, initials,  [150000,55000], time, 2,
#                         exp_data=exp_datas[0], exp_time=exp_time, ss=True, t100a=False, ptpD=False, ci='sd')
#
#     plt_param_cis_fig3c(ax[0, 1], model_fxns, sim_data[1], params_constants, initials,  [150000,55000], time, 2,
#                         exp_data=exp_datas[0], exp_time=exp_time, ss=True, t100a=False, ptpD=False, ci='sd')
#
#     plt_param_cis_fig3c(ax[0, 2], model_fxns, sim_data[2], params_constants, initials,  [150000,55000], time, 2,
#                         exp_data=exp_datas[0], exp_time=exp_time, ss=True, t100a=False, ptpD=False, ci='sd')
#
#     plt_param_cis_fig3c(ax[1, 0], model_fxns, sim_data[0], params_constants, initials,  [150000,55000], time, 2,
#                         exp_data=exp_datas[1], exp_time=exp_time, ss=True, t100a=False, ptpD=False, ci='sd')
#
#     plt_param_cis_fig3c(ax[1, 1], model_fxns, sim_data[1], params_constants, initials,  [150000,55000], time, 2,
#                         exp_data=exp_datas[1], exp_time=exp_time, ss=True, t100a=False, ptpD=False, ci='sd')
#
#     plt_param_cis_fig3c(ax[1, 2], model_fxns, sim_data[2], params_constants, initials,  [150000,55000], time, 2,
#                         exp_data=exp_datas[1], exp_time=exp_time, ss=True, t100a=False, ptpD=False, ci='sd')
#
#     plt.show()
