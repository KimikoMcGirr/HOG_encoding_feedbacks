...
filename = 'b3_1D_a3s_map2k_branches_190111.txt'
...
def a3s_1D_branched(inits,t,params_constants,params):
    Sho1, Sln1, Pbs2, Hog1, X = inits
    Sho1_t, Sln1_t, Pbs2_t, Hog1_t, sho1_on, sln1_on, s = params_constants
    beta_s, alpha_sho1, alpha_sln1, k1, k3, ksho1, ksln1, k7, s9, k2, k4, k6, k8, d10, K_1, K_3, K_sho1, K_sln1, K_7, K_2, K_4, K_6, K_8 = params #22
    ...
    dSho1 = (s/(1+X/beta_s)) * (((k1+alpha_sho1*Hog1)*Sho1_I)/(K_1+Sho1_I)) - (k2*Sho1/(K_2+Sho1))
    dSln1 = (s/(1+X/beta_s)) * (((k3+alpha_sln1*Hog1)*Sln1_I)/(K_3+Sln1_I)) - (k4*Sln1/(K_4+Sln1))
    dPbs2 = sho1_on*(((ksho1*Sho1)*Pbs2_I)/((K_sho1)+Pbs2_I)) + sln1_on*(((ksln1*Sln1)*Pbs2_I)/((K_sln1)+Pbs2_I)) - (k6*Pbs2/(K_6+Pbs2))
    dHog1 = (((k7*Pbs2)*Hog1_I)/(K_7+Hog1_I)) - (k8*Hog1/(K_8+Hog1))
    dX = s9*Hog1 - d10*X
...
def simulate_wt_experiment(inits, params_constants, learned_params, time):
    ...
    #solve odes:
    odes = odeint(a3s_1D_branched, inits, time, args=(params_constants, learned_params))
...
def simulate_t100a_experiment(inits, params_constants, learned_params, time):
    ...
    # parameters to be kept constant - params_constants
    # parameters to be learned - learned_params
    beta_s, alpha_sho1, alpha_sln1, k1, k3, ksho1, ksln1, k7, s9, k2, k4, k6, k8, d10, K_1, K_3, K_sho1, K_sln1, K_7, K_2, K_4, K_6, K_8  = learned_params
    learned_params = beta_s, 0, 0, k1, k3, ksho1, ksln1, k7, 0, k2, k4, k6, k8, d10, K_1, K_3, K_sho1, K_sln1, K_7, K_2, K_4, K_6, K_8
    #solve odes:
    odes = odeint(a3s_1D_branched, inits, time, args=(params_constants, learned_params))
