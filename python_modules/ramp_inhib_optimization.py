...
def get_odes_predict(model_fxns, ts_steps, inhib, param_sets, inits, total_protein, time, selection=''):
    traces = np.zeros([len(param_sets), len(time)])
    for params in param_sets:
        ss_inits = run_ss(model_fxns.inhib, inits, total_protein, params)
        data = simulate_wt_experiment(model_fxns.inhib, ss_inits, total_protein, 0, params, time, run_type=['rand_off', ts_steps, inhib])
        traces += data[:,2]/total_protein[2]*100
    return traces/len(param_sets)
...
# def get_average(fxn, ts_steps, inhib, param_sets, inits, total_protein, time, selection=''):
#     traces = get_odes_predict(fxn, ts_steps, inhib, param_sets, inits, total_protein, time, selection)
#     return traces/len(param_sets)
...
def get_differentiating_ramp(num_runs, time,
    ...
    for i in range(num_runs):
        z = range(10)
        inhib = np.zeros(len(ts_steps))
        inhib_step = rand.choice(range(len(ts_steps)))
        inhib[inhib_step:] = 1


        ave1 = get_odes_predict(model_fxns1, ts_steps, inhib, model_params1, inits1, total_protein1, time, selection)
        ave2 = get_odes_predict(model_fxns2, ts_steps, inhib, model_params2, inits2, total_protein2, time, selection)
